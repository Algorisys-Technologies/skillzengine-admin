...
export class CreatetestComponent implements OnInit {
  ...
  testForm = this.fb.group({
    ...
    category: [
      { value: ["Select Category"], disabled: false },
      Validators.required,
    ],
  ...
  onSubmit() {
    // Assign random questions if not manually selected
    if (this.mqIds.length === 0) {
      this.mqIds = this.getRandomQuestions(
        this.testForm.controls["numQuestions"].value
      );
    }
  ...
  private create() {
    ...
    this.testsService.add(this.testForm.getRawValue()).subscribe(
      (response: Response) => {
  ...
  // Method to select random questions from available questions
  private getRandomQuestions(num: number): any[] {
    const shuffled = this.questions.sort(() => 0.5 - Math.random()); // Shuffle questions randomly
    return shuffled.slice(0, num).map((q) => q._id); // Return random question IDs
  }
