...
export class EdittestComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.selectedQuestions.length === 0) {
      const numQuestions = this.testForm.controls["numQuestions"].value;
      this.selectedQuestions = this.getRandomQuestions(numQuestions);
  ...
  private edit() {
    ...
    this.testsService.updateTest(this.testForm.getRawValue()).subscribe(
      (response: Response) => {
  ...
  populateTestForm(data) {
    ...
    this.testForm.controls["numQuestions"].setValue(data[0]["qIds"].length);
  ...
  private getRandomQuestions(num: number): any[] {
    ...
    return shuffled.slice(0, num).map((q) => q._id);
  }
